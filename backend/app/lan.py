...
def _validate_scope(scope: str) -> str:
    ...
    Raises 400 for obviously malformed input before it reaches DB resolution.
    """
    if not scope:
        return "all"
    if scope == "all":
        return scope
    if scope.startswith("group:") or scope.startswith("country:"):
        # Sanity check length -- prevents extremely long inputs
        if len(scope) > 256:
            raise HTTPException(
                status_code=400,
                detail="scope parameter too long (max 256 characters)."
            )
        return scope
    raise HTTPException(
        status_code=400,
...
def lan_alerts(
    ...
    return [
        a for a in alerts
        if node_country.get(a.node_name, "") in countries
    ]
